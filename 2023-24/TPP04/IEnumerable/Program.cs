...
namespace IEnumerable
{
    class Program
    {
        ...
        */
        public static IEnumerable<T> MatchPos<T> (IEnumerable<T> lista1, IEnumerable<T> lista2)
        {
            List<T> lista = new List<T>();
            var valor1 = lista1.GetEnumerator();
            var valor2 = lista2.GetEnumerator();
            while (valor1.MoveNext() && valor2.MoveNext())
            {
                if (valor1.Current.Equals(valor2.Current))
                {
                    lista.Add(valor1.Current);
                }
            }
            return lista;
